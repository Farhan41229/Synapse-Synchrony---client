...
const BookmarkedBlogs = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await blogService.getMyBookmarkedBlogs();
  ...
  // Unbookmark blog mutation
  const unbookmarkMutation = useMutation({
    mutationFn: blogService.toggleBookmarkBlog,
    onSuccess: () => {
      queryClient.invalidateQueries(['bookmarked-blogs']);
      toast.success('Removed from bookmarks');
    },
    onError: (error) => {
      toast.error(error.response?.data?.message || 'Failed to unbookmark blog');
    },
  });
  ...
  // Get most liked blog's like count
  const mostLiked = blogs?.length > 0 
    ? Math.max(...blogs.map(blog => blog.likeCount || blog.likes?.length || 0))
    : 0;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {!isLoading && blogs && (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8" data-aos="fade-up">
            <div className="p-4 bg-gradient-to-r from-[#04642a]/10 to-[#15a33d]/10 dark:from-[#04642a]/20 dark:to-[#15a33d]/20 rounded-xl border border-[#04642a]/20">
              <BookmarkX className="w-6 h-6 text-[#04642a] mb-2" />
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16" data-aos="fade-up">
            ...
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
              {searchQuery ? 'No bookmarked blogs found' : "You haven't bookmarked any blogs yet"}
            </h3>
